...
public class Verifica {
	...
	Cliente cliente = new Cliente("", "", "", "", "", true, 0, 0, 0, 0);
	...
	public boolean CrearPublicacion(String nombre, int precio, boolean temporal, boolean comprar, String zona,
			String direccion, int ambientes, String estado, int id_propietario) {
					...
					if (zona.length() > 2) {
						if (direccion.length() > 2) {
							if (ambientes > 0) {
								...
								ambientes = Integer.parseInt(
										JOptionPane.showInputDialog("Error debe ingresar al menos 1 ambiente"));
							}
						} else {
							direccion = JOptionPane
									.showInputDialog("Error la direccion debe tener mas de 2 caracteres");
						}
					} else {
	...
	public boolean crearTransaccion(String nro_tarjeta, String clave_seguridad, String nombretarjeta, int cliente_id,
			int publicacion_id, double precio) {
		boolean salir = false;
		...
		do {
			if (comentario.length() <= 100) {
				if (estrellas.length() == 1 && Integer.parseInt(estrellas) >= 1 && Integer.parseInt(estrellas) <= 5) {
					...
					estrellas = JOptionPane
							.showInputDialog("El total de estrellas solo puede ser un numero del 1 al 5");
				}
			} else {
